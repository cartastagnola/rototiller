...
def processing(stdscr, screen_state: TYPE.ScreenState, keyboard_state: TYPE.KeyboardState,
    ...
    while True:
        ...
        if key >= 0:
            ...
            keyboard_state.keys.append(chr(key))
        ...
        match active_scope.mode:
            ...
            case TYPE.ScopeMode.VISUAL:
                ...
                elif key == 22 or key == ord('p'):  # ctrl-v
                    keyboard_state.paste = True
                elif key == curses.KEY_DC or key == curses.KEY_BACKSPACE or key == ord('d'):  # delete
                    keyboard_state.delete = True
...
def default_execution(screen_state: TYPE.ScreenState, keyboard_state: TYPE.KeyboardState,
                      active_scope: TYPE.Scope):
    ...
    ## now: exec_child when there are child
    ## or exec_own when there are no child
    print(f"active scope: {active_scope.name}, args NOE and exe: {active_scope.exec_child} ")
    print(f"main: {active_scope.main_scope.name},  ")
    if keyboard_state.enter is True:
        active_scope.exec_child(*screen_state.scope_exec_args)
        return
    if keyboard_state.esc is True:
...
def block_band_execution(screen_state: TYPE.ScreenState, keyboard_state: TYPE.KeyboardState,
                         active_scope: TYPE.Scope):
